...
struct AhoCorasick {
    ...
    void build_failTree(){
        ...
        f(i,1,sz){
            if(!trie[i].ids.empty()){
                cnt.add(in[i]+1,trie[i].ids.size());
                cnt.add(out[i]+2,-trie[i].ids.size());
            }
           // cout<<i<<" "<<trie[i].exit_link<<" "<<cnt.sumPrefix(in[i]+1)<<ln;
        }
...
void solve(int test_case) {
    ...
    while(q--){
        ...
        if(ch == '?'){
            ...
            int x; cin>>x;
            if(exist[x-1] == 0){
                ac.cnt.add(ac.in[mp[x-1]]+1,1);
                ac.cnt.add(ac.out[mp[x-1]]+2,-1);
            }
            exist[x-1] = 1;
        }else{
            int x; cin>>x;
