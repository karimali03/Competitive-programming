...
void solve(int test_case) {
    int n,k,m; cin>>n>>k>>m;
    int s = n*k; int ss = n*k + 1 , t = n*k+2;
    vii v(n,vi(k));
    f(i,0,n) f(j,0,k) cin>>v[i][j];
    Dinic st;
    st.init(s,t);
    st.addEdge(ss,s,INF);
    for(int i = 0 ; i < k ; i++) st.addEdge(s,i,v[0][i]);
    for(int i = (n-1)*k ;  i < n*k ; i++) st.addEdge(i,t,v[n-1][i - (n-1)*k]);
    for(int i  = 0; i < m ; i++){
        int x,y,z; cin>>x>>y>>z; x--,y--,z--;
        st.addEdge(x * k + y , (x+1) * k + z , v[x+1][z]);
    }    
    cout<<st.maxFlow()<<ln;
}
