...
void solve(int tc = 0) {
    ...
    // Right nodes to Sink
    for (int i = 0; i < R; i++) {
        mcmf.add_edge(i + L, T, 1, 0);
    }

    // Input edges: Left x to Right y, weight w → cost = -w
    for (int i = 0; i < n; i++) {
        int x, y, w;
        cin >> x >> y >> w;
        --x, --y;
        mcmf.add_edge(x, y + L, 1, -w, i);  // ID = i for tracking
    }

    auto res = mcmf.solve(S,T);
    cout<<res.first<<" "<<res.second<<ln;
    auto ret = mcmf.getAllFlowPathsWithAmounts(S,T);
    for(auto it : ret){
