...
void solve(int test_case) {
    ...
    for(int i = 0 ;i < n ; i++) st[s[i]-'a'].update(i,i,1);
    for(int i = 0 ; i < q ; i++){
        int l,r,t; cin>>l>>r>>t; l--,r--;
        vi cnt(26);
        for(int x = 0 ; x < 26 ; x++) cnt[x] = st[x].query(l,r).val;
        int a = l;
        if(t){
            for(int x = 0 ; x < 26 ; x++){
                st[x].update(a,a+cnt[x]-1,1);
                st[x].update(l , a-1 , 0);
                st[x].update(a+cnt[x] , r , 0);
                a += cnt[x];
            }
        }else{
            for(int x = 25 ; x >= 0 ; x--){
