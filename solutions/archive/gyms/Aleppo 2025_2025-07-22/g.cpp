...
struct XorBasis {
    ...
    // Now this will correctly detect if there exists a non-empty subset with XOR = 0
    bool XrZero() const {
...
bool explore(int v,int cnt,XorBasis st,int n){
    ...
    if(v == comp[n-1] && st.XrZero()) return true;
    if(cnt >= 20 || (v == comp[n-1])) return false;
    for(auto x : adj2[v]){
